...
# def _count_turns(entry: dict) -> int:
#     """Count shard_revealed events in the trace (= number of shards revealed)."""
#     trace = entry.get("trace", [])
#     return sum(
#         1 for e in trace
#         if e.get("role") == "log"
#         and isinstance(e.get("content"), dict)
#         and e["content"].get("type") == "shard_revealed"
#     )


# def classify_entry(entry: dict, task_type: str, include_success: bool = False) -> Optional[FailureResult]:
#     """Return a FailureResult for the entry, or None for successes when include_success=False."""
#     score = entry.get("score")
#     is_success = score not in (0, 0.0, None)
...
#     min_turns: int = 0,
#     max_turns: int = 9999,
#     out_path: str = "category_task_ids.json",
# ) -> None:
#     """
#     Process JSONL files and write a JSON file mapping
#     {task_type: {category: [task_ids]}} for use by run_simulations.py.

#     For task_ids that appear multiple times (e.g. the same task re-run),
#     the category is decided by majority vote across all runs in the turn range.
#     """
#     # Collect (task_id, category) pairs per detected task_type
#     from collections import defaultdict
#     task_votes: dict[str, dict[str, list[str]]] = defaultdict(lambda: defaultdict(list))
#     # task_votes[task_type][task_id] = [cat, cat, cat, ...]
...
#         tt = _detect_task_type(entries) if task_type == "auto" else task_type
...
#                 skipped_turns += 1
#                 continue
#             result = classify_entry(entry, tt, include_success=True)
#             if result and result.task_id:
#                 task_votes[tt][result.task_id].append(result.category)

#     # Majority-vote: pick the most common category per task_id
#     out: dict[str, dict[str, list[str]]] = {}
#     total_unique = 0
#     for tt, id_votes in task_votes.items():
#         cat_to_ids: dict[str, list[str]] = defaultdict(list)
#         for task_id, votes in id_votes.items():
#             majority = Counter(votes).most_common(1)[0][0]
#             cat_to_ids[majority].append(task_id)

#         # Sort task_ids numerically where possible
#         def _sort_key(tid: str) -> tuple:
#             suffix = tid.rsplit("/", 1)[-1]
#             return (int(suffix),) if suffix.isdigit() else (float("inf"), tid)

#         out[tt] = {cat: sorted(ids, key=_sort_key) for cat, ids in sorted(cat_to_ids.items())}
#         total_unique += sum(len(v) for v in out[tt].values())

#     with open(out_path, "w", encoding="utf-8") as f:
#         json.dump(out, f, indent=2, ensure_ascii=False)

#     print(f"\nExported task IDs → {out_path}")
#     print(f"  Records processed : {total_records}  (skipped outside turns [{min_turns},{max_turns}]: {skipped_turns})")
#     print(f"  Unique task_ids   : {total_unique}")
#     for tt, cats in out.items():
#         print(f"  [{tt}]")
...
#     )
#     parser.add_argument(
#         "--export-task-ids", metavar="FILE",
#         help="Write {task_type: {category: [task_ids]}} JSON for use by run_simulations.py.",
#     )
#     parser.add_argument(
#         "--min-turns", type=int, default=0,
#         help="Only include records with >= this many turns (for --export-task-ids).",
#     )
#     parser.add_argument(
#         "--max-turns", type=int, default=9999,
#         help="Only include records with <= this many turns (for --export-task-ids).",
#     )

#     args = parser.parse_args()

#     if args.export_task_ids:
#         export_task_ids(
#             args.paths,
#             task_type=args.task_type,
#             min_turns=args.min_turns,
#             max_turns=args.max_turns,
#             out_path=args.export_task_ids,
#         )
#         return
